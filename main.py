...
if __name__ == "__main__":
    ...
    print("Активных пропусков: {}".format(
    ...
    lst_in_storage = [
        visit.passcard.owner_name for visit in Visit.objects.filter(
            leaved_at=None)
    ]
    print("Список людей в хранилище:", lst_in_storage)

    print("Число визитов в хранилище человека с пропуском id=1({1}): {0}".format(
        Visit.objects.filter(
        passcard__id=passcard.pk).count(), passcard.owner_name))
    print('Визиты дольше 1000 мин:', len([
        item for item in Visit.objects.all() if item.is_visit_long(
            minutes=60)
        ]))
